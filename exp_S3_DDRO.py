...
def visualize_scores(train_noise, mu, Sigma, rho_gauss, rho_ddro):
    ...
    plt.figure(figsize=(10, 6))
    sns.histplot(scores, stat="density", bins=50, color="skyblue", label="Empirical Data", alpha=0.6)
    
    x = np.linspace(0, max(scores), 100)
    df = train_noise.shape[1]
    plt.plot(x, chi2.pdf(x, df), 'r-', lw=2, label=f"Theoretical Chi2 (df={df})")
    
    plt.axvline(rho_gauss, color='r', linestyle='--', label=f"Gaussian 95% ({rho_gauss:.1f})")
    plt.axvline(rho_ddro, color='b', linestyle='--', label=f"DDRO 95% ({rho_ddro:.1f})")
    
    plt.title("Non-conformity Scores: Empirical vs Theoretical")
    plt.xlabel("Score (Mahalanobis Distance^2)")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.show()
def calculate_robust_parameters(train_noise, cal_noise, H, C_gen, alpha, epsilon=0.05):
    print(f"\n[Calibration] Target Violation < {epsilon*100}%")
    
    # 1. 形状估计
    mu = np.mean(train_noise, axis=0)
    Sigma = np.cov(train_noise, rowvar=False) + 1e-8 * np.eye(train_noise.shape[1])
    
    Sigma_sqrt = np.real(sqrtm(Sigma))
    Sigma_inv = np.linalg.inv(Sigma) # 逆矩阵用于计算 score
    
    # 2. 分数计算 (Calibration)
    scores = []
    for i in range(cal_noise.shape[0]):
        diff = cal_noise[i] - mu
        score = diff.T @ Sigma_inv @ diff
        scores.append(score)
    scores = np.array(scores)
    ...
    df = train_noise.shape[1] 
    rho_gauss = chi2.ppf(1 - epsilon, df)
    print(f"  > Theoretical Gaussian Rho : {rho_gauss:.2f}")
    print(f"  > Data-Driven DDRO Rho     : {rho_ddro:.2f}")
    ...
    return omega_margin, line_margin, line_shift, mu, rho_ddro, W, rho_gauss

def solve_affine_dispatch(d_forecast, C_gen, H, gen_max, branch_limit, alpha, 
                          omega_margin, line_margin, line_shift, mu, prices): 
    # 保持不变，这就是那个完美的仿射调度函数
    n_gen = len(gen_max)
    x = cp.Variable(n_gen)
    total_mu = np.sum(mu)
    ...
    constraints = [
        cp.sum(x) == np.sum(d_forecast) + total_mu,
        x + alpha * total_mu + alpha * omega_margin <= gen_max,
        x + alpha * total_mu - alpha * omega_margin >= 0,
        H @ (C_gen @ x - d_forecast) + line_shift <= branch_limit - line_margin,
        H @ (C_gen @ x - d_forecast) + line_shift >= -branch_limit + line_margin
    ]
...
def run_experiment():
    print("=== 开始第三章实验：Deep Diagnostics Version (System Shock) ===")
    data = load_data()
    C_gen, H = get_true_topology(data['mpc_raw'])
    ...
    # --- 参数计算 ---
    omega_ddro, line_ddro, line_shift, mu, rho_ddro, W, rho_gauss = calculate_robust_parameters(
        train_noise, cal_noise, H, C_gen, data['alpha'], epsilon=0.05
    )
    
    # A. Gaussian (使用相同的 mu, W 逻辑, 但用 rho_gauss)
    df = train_noise.shape[1]
    Sig_g = np.cov(train_noise, rowvar=False) + 1e-8*np.eye(df)
    Sig_sqrt_g = np.real(sqrtm(Sig_g))
    
    omega_gauss = np.sqrt(rho_gauss) * np.linalg.norm(Sig_sqrt_g @ np.ones(df))
    line_gauss = np.sqrt(rho_gauss) * np.linalg.norm(W @ Sig_sqrt_g, axis=1)
    
    # C. Traditional RO (Empirical Worst Case)
    # 针对 centered data 的最大波动
    centered_train = train_noise - mu
    omega_ro = np.max(np.abs(np.sum(centered_train, axis=1)))
    line_ro = np.max(np.abs(centered_train @ W.T), axis=0)
    
    print(f"\n[Margins Comparison - Total Power Imbalance]")
    print(f"  Gaussian Margin : {omega_gauss:.2f} MW (Too small for shocks)")
    print(f"  DDRO Margin     : {omega_ddro:.2f} MW (Captures tail)")
    print(f"  RO Margin       : {omega_ro:.2f} MW (Empirical max)")

    # 逻辑检查
    if omega_ddro > omega_gauss * 1.5:
        print("  >>> 成功：DDRO 显著大于 Gaussian，预期 Gaussian 将会违约。")
    ...
    print(f"\n{'Method':<15} | {'Viol%':<8} | {'Infeas':<6} | {'Cost':<8} | {'LineVio':<6}")
    print("-" * 60)
    ...
    for name, (m_omega, m_line) in methods.items():
        ...
        for t in range(len(test_forecast)):
            x_star, feasible, cost_val = solve_affine_dispatch(
                test_forecast[t], C_gen, H, data['gen_max'], data['branch_limit'],
                data['alpha'], m_omega, m_line, line_shift, mu, gen_prices
            )
            
            if not feasible:
                infeasible += 1
                violations += 1 # Infeasible 视为违约
                continue
            ...
            # 真实校验
            xi_real = test_real[t] - test_forecast[t]
            p_real = x_star + data['alpha'] * np.sum(xi_real)
            ...
            if is_line_vio: line_vio_count += 1
            if is_gen_vio or is_line_vio:
                violations += 1
        
        rate = violations / len(test_forecast) * 100
        avg_cost = total_cost / valid_samples if valid_samples > 0 else 0
        
        print(f"{name:<15} | {rate:<8.2f} | {infeasible:<6} | {avg_cost:<8.0f} | {line_vio_count:<6}")
