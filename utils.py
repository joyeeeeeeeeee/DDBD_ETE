...
def load_data(filepath='experiment_data_case39.mat'):
    ...
    # ==================================================
    # 🔴 终极微调：隐形杀手 (Stealthy Shock)
    # ==================================================
    np.random.seed(42) 
    n_samples, n_bus = load_forecast.shape
    
    # 1. 极度安静的基础底噪 (0.2%)
    # 这会导致 Sigma 极小，Gaussian 会因此变得非常轻敌（Margin 很小）
    base_sigma = 0.002 * load_forecast
    noise = np.random.randn(n_samples, n_bus) * base_sigma
    
    # 2. 极其稀疏但毁灭性的冲击 (Rare but Massive)
    # 概率降到 2% (非常稀疏，几乎不影响 Covariance 计算)
    n_spikes = int(n_samples * 0.01) 
    if n_spikes < 1: n_spikes = 1
    
    spike_time_idx = np.random.choice(n_samples, n_spikes, replace=False)
    
    # 幅度提升到 40% (一击毙命)
    for t in spike_time_idx:
        direction = np.random.choice([-1, 1])
        # 40% 的同向波动
        mag = 0.40
        noise[t, :] += direction * mag * load_forecast[t, :]
    
    load_noise = noise
    load_real = load_forecast + load_noise
    
    # 3. 扩容 (1.35倍)
    # 必须给 DDRO 留出活路。40% 的波动需要巨大的备用。
    # 1.35x 大概能提供 35% 的额外容量，接近极限但有解
    gen_info = mpc_raw['gen']
    branch_info = mpc_raw['branch']
    
    gen_max = gen_info[:, 8].astype(float) * 1.35
    branch_limit = branch_info[:, 5].astype(float) 
    ...
    print(f"数据重构完成：底噪(0.2%)，稀疏毁灭冲击(6%, 35%)，容量(1.35x)。")
    # ==================================================
