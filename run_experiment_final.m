function [data_load, data_price]= run_experiment_stratified()
    %% 强制分层生成数据：确保训练集和测试集都有尖峰
    clear; clc;
    rng(2024); % 新种子

    %% 1. Case39 扩容 (保持不变)
    mpc = case39_mod(); 
    % 关键：给够 Headroom，防止 RO 无解
    mpc.gen(:, 9) = mpc.gen(:, 9) * 2.0;  % 发电机容量翻倍
    mpc.branch(:, 6) = mpc.branch(:, 6) * 2.0; % 线路容量翻倍

    %% 2. 生成预测负荷 (保持不变)
    days = 90; H = 24; N_samples = days * H;
    ...
    % 定义数据集分割点 (对应 Python 的 50% Train, 20% Cal, 30% Test)
    n_train = floor(N_samples * 0.5);
    n_cal = floor(N_samples * 0.2);
    n_test = N_samples - n_train - n_cal;
    
    idx_train = 1:n_train;
    idx_cal = (n_train+1):(n_train+n_cal);
    idx_test = (n_train+n_cal+1):N_samples;
    
    for i = 1:n_load
        idx = load_buses(i);
        
        % 1. 生成全时段的基础底噪 (非常小，0.5%)
        % 这会让 Gaussian 算出来的 Sigma 很小
        base_sigma = 0.005 * Pd_forecast(:, idx);
        noise_vec = randn(N_samples, 1) .* base_sigma;
        
        % 2. 强制在每个集合中注入尖峰 (20% 幅度)
        % 尖峰概率 10%
        prob_spike = 0.10;
        spike_mag = 0.20; 
        
        % 函数：局部注入
        noise_vec(idx_train) = inject_spikes(noise_vec(idx_train), Pd_forecast(idx_train, idx), prob_spike, spike_mag);
        noise_vec(idx_cal)   = inject_spikes(noise_vec(idx_cal),   Pd_forecast(idx_cal, idx),   prob_spike, spike_mag);
        noise_vec(idx_test)  = inject_spikes(noise_vec(idx_test),  Pd_forecast(idx_test, idx),  prob_spike, spike_mag);
...
function vec_out = inject_spikes(vec_in, load_in, prob, mag)
    % 辅助函数：在给定向量中注入尖峰
    n = length(vec_in);
    n_spikes = round(n * prob);
    spike_idx = randperm(n, n_spikes);
    
    % 注入单向正尖峰 (abs)
    spike_val = mag * load_in(spike_idx);
    vec_in(spike_idx) = vec_in(spike_idx) + abs(spike_val);
    vec_out = vec_in;
end

function mpc = case39_mod()
    mpc = case39(); % 你的原始 case39
    % 梯级电价
    ng = size(mpc.gen, 1);
    new_costs = zeros(ng, 7);
    new_costs(:, 1) = 2; new_costs(:, 4) = 3; new_costs(:, 5) = 0.001;
...
%% generator cost data (Dummy, will be overwritten by mod)
mpc.gencost = [
